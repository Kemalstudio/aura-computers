            ...
            <div class="col-lg-3">
                <aside style="position: sticky; top: calc(var(--navbar-height, 70px) + 1.5rem); z-index: 1000;">
                    <form id="filterForm" action="{{ route('catalog.index') }}" method="GET" class="filters-sidebar">
                        ...
                        {{-- Page is handled by pagination links --}}
                                        ...
                                        foreach ($activeDynamicFiltersConfig as $filterKey => $filterItemDataForSummary) {
                                            ...
                                            if ($requestValue !== null && $requestValue !== '' && (!is_array($requestValue) || !empty(array_filter($requestValue)))) {
                                                ...
                                                if (is_array($requestValue)) {
                                                    $labels = [];
                                                    foreach(array_filter($requestValue) as $val) { // Filter out empty values from array
                                                ...
                                                // Unit is already part of the label in $optionsForSummary from controller
                                                if ($displayValue) {
...
@push('styles')
    ...
    <style>
        :root {
            --navbar-height: 60px; /* Adjust if your navbar height is different */
            --bs-primary-rgb: {!! implode(', ', sscanf(config('yourconfig.primary_color', '#0d6efd'), "#%02x%02x%02x")) !!}; /* Example, get from config or define directly */
            --bs-primary: {{ config('yourconfig.primary_color', '#0d6efd') }};
            --bs-primary-bg-subtle: #cfe2ff; /* Example */
        ...
        #appNavbar { z-index: 1030; }
        .dropdown-menu.animate { animation-duration: 0.25s; animation-fill-mode: both; }
        .dropdown-menu.animate.slideIn { animation-name: slideInSubtle; }
        @keyframes slideInSubtle { 0% { transform: translateY(-5px); opacity: 0; } 100% { transform: translateY(0); opacity: 1; } }
        .list-group-categories-styled .list-group-item { border: none; padding: 0; position: relative; }
        .list-group-categories-styled .list-group-item .category-link,
        .list-group-categories-styled .list-group-item .subcategory-link {
            padding: 0.6rem 1rem; font-size: 0.875rem; text-decoration: none; color: var(--bs-list-group-color);
            display: flex; align-items: center; width: 100%; transition: background-color 0.15s ease-in-out, color 0.15s ease-in-out;
            border-radius: var(--bs-border-radius);
        }
        .list-group-categories-styled .list-group-item .category-link:hover,
        .list-group-categories-styled .list-group-item .subcategory-link:hover { background-color: var(--bs-list-group-hover-bg); color: var(--bs-list-group-hover-color); }
        .list-group-categories-styled .list-group-item .category-link.active,
        .list-group-categories-styled .list-group-item .subcategory-link.active { color: var(--bs-list-group-active-color); background-color: var(--bs-list-group-active-bg); font-weight: 500; }
        .list-group-categories-styled .list-group-item .category-link.active-parent { background-color: var(--bs-primary-bg-subtle); color: var(--bs-primary); font-weight: 500; }
        .list-group-categories-styled .list-group-item .category-icon { flex-shrink: 0; width: 20px; height: 20px; object-fit: contain; }
        .list-group-categories-styled .list-group-item .category-text { flex-grow: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin-right: 0.5rem; }
        .list-group-categories-styled .list-group-item .submenu-indicator { transition: transform 0.2s ease-in-out; font-size: 0.8em; }
        .list-group-categories-styled .list-group-item.has-submenu .submenu {
            display: block; visibility: hidden; opacity: 0; position: absolute; left: 100%; top: -1px; min-width: 250px; z-index: 1025;
            padding-left: 0; list-style: none; background-color: var(--bs-list-group-bg, #fff); border: 1px solid var(--bs-list-group-border-color, rgba(0,0,0,.1));
            margin-top: 0; border-radius: 0 var(--bs-border-radius) var(--bs-border-radius) 0; box-shadow: 0.125rem 0.25rem 0.5rem rgba(0,0,0,.1);
            transform: translateX(10px); transition: opacity 0.2s ease-out, visibility 0s linear 0.2s, transform 0.2s ease-out;
        }
        .list-group-categories-styled .list-group-item.has-submenu:hover > .submenu,
        .list-group-categories-styled .list-group-item.has-submenu .submenu:hover { visibility: visible; opacity: 1; transform: translateX(0); transition-delay: 0s,0s,0s; }
        .list-group-categories-styled .list-group-item.has-submenu .submenu.submenu-left { left: auto; right: 100%; transform: translateX(-10px); border-radius: var(--bs-border-radius) 0 0 var(--bs-border-radius); }
        .list-group-categories-styled .list-group-item.has-submenu .submenu.submenu-left .list-group-item { padding-left: 0.5rem; }
        .list-group-categories-styled .list-group-item.has-submenu:hover > .submenu.submenu-left,
        .list-group-categories-styled .list-group-item.has-submenu .submenu.submenu-left:hover { transform: translateX(0); }

        .filter-section.card { border: 1px solid #e9ecef; }
        .filter-section-title { cursor: pointer; font-size: 0.95rem; padding: 0.6rem 1rem; }
        .filter-section-title:hover { background-color: #f8f9fa; }
        .filter-section-body { padding: 0.75rem 1rem; max-height: 250px; overflow-y: auto; font-size: 0.875rem; }
        .filter-section-body .form-check { margin-bottom: 0.5rem; padding-left: 1.9em; }
        .filter-section .filter-arrow { transition: transform 0.2s ease-in-out; }
        .filter-section-title[aria-expanded="false"] .filter-arrow { transform: rotate(-90deg); }
        #price-slider { margin-top: 1rem; margin-bottom: 0.25rem; }
    </style>
@endpush
