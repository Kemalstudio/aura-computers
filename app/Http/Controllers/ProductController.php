...
class ProductController extends Controller
{
    public function index(Request $request): View
    {
        ...
        if ($selectedCategory && $selectedCategory->attributes->isNotEmpty()) {
            ...
            foreach ($categoryAttributes as $attribute) {
                ...
                $tempMin = $request->query('min_price');
                $tempMax = $request->query('max_price');
                ...
                foreach ($categoryAttributes as $otherAttr) {
                    ...
                    $otherKey = 'attr_' . $otherAttr->slug;
                    $otherVal = $request->input($otherKey);
                    ...
                    if ($otherVal !== null && $otherVal !== '' && $otherCol) {
                        $optionsQueryContextForThisAttribute->whereHas('attributeValues', function (Builder $q) use ($otherAttr, $otherVal, $otherCol) {
                            ...
                            if (is_array($otherVal)) {
                                $c = array_filter($otherVal, fn($v) => $v !== null && $v !== '');
                                if (!empty($c)) $q->whereIn($otherCol, $c);
                            } else {
                                $q->where($otherCol, $otherVal);
                            }
                        });
                    }
                }
                ...
                if ($finalOptionsSource->isEmpty()) {
                    Log::debug("Skipping filter '{$attribute->name}': no options.");
                    continue;
                }
                ...
                if (!empty($formattedOptions)) {
                    ...
                    $availableSpecificFiltersData[$filterRequestKey] = [
                        ...
                        'defined_options' => $attribute->options,
                        'request_key' => $filterRequestKey,
                        'unit' => $attribute->unit,
                    ];
                } else {
                    Log::debug("No formatted options for '{$attribute->name}', not adding filter.");
                }
                ...
                if ($requestValue !== null && $requestValue !== '') {
                    ...
                    $productsQuery->whereHas('attributeValues', function (Builder $q) use ($attribute, $requestValue, $valueColumn) {
                        ...
                        if (is_array($requestValue)) {
                            $c = array_filter($requestValue, fn($v) => $v !== null && $v !== '');
                            if (!empty($c)) $q->whereIn($valueColumn, $c);
                        } else {
                            $q->where($valueColumn, $requestValue);
                        }
                    });
                }
            }
        } else {
            Log::info($selectedCategory ? "Category '{$selectedCategory->name}' has no filterable attributes (or attributes at all)." : "No category selected, dynamic filters skipped.");
        }
        ...
        if ($overallMinPrice == $overallMaxPrice && $overallMaxPrice > 0) {
            $overallMinPrice = max(0, floor($overallMinPrice * 0.85));
            $overallMaxPrice = ceil($overallMaxPrice * 1.15);
        } elseif ($overallMinPrice >= $overallMaxPrice) {
            $tempMax = $overallMaxPrice;
            $overallMinPrice = $tempMax > 0 ? max(0, floor($tempMax * 0.85)) : 0;
            if ($tempMax == 0 && $overallMinPrice == 0) $overallMaxPrice = 1000;
            else if ($tempMax == 0) $overallMaxPrice = $overallMinPrice > 0 ? $overallMinPrice + 100 : 1000;
            else $overallMaxPrice = ceil($tempMax * 1.15);
        }
        if ($overallMinPrice > $overallMaxPrice && $overallMaxPrice > 0) $overallMinPrice = floor($overallMaxPrice * 0.9);
        elseif ($overallMinPrice > $overallMaxPrice && $overallMaxPrice == 0) {
            $overallMinPrice = 0;
            $overallMaxPrice = 100;
        }
        if ($overallMinPrice == $overallMaxPrice && $overallMinPrice >= 0) $overallMaxPrice = $overallMinPrice + max(100, floor($overallMinPrice * 0.1));
        ...
        switch ($sortBy) {
            ...
            case 'name_desc':
                ...
                break;
            case 'price_asc':
                $productsQuery->orderBy(DB::raw('COALESCE(sale_price, price)'), 'asc');
                break;
            case 'price_desc':
                $productsQuery->orderBy(DB::raw('COALESCE(sale_price, price)'), 'desc');
                break;
            case 'created_at_asc':
        ...
        if ($availability === 'in_stock') $brandsContextQuery->where('quantity', '>', 0);
        elseif ($availability === 'out_of_stock') $brandsContextQuery->where(fn($q) => $q->where('quantity', '<=', 0)->orWhereNull('quantity'));
        ...
        if ($selectedCategory && $selectedCategory->attributes->isNotEmpty()) {
            $catAttrsForBrand = $selectedCategory->attributes()->where('is_filterable', true)->get();
            foreach ($catAttrsForBrand as $attrForBrand) {
                $key = 'attr_' . $attrForBrand->slug;
                ...
                if ($val !== null && $val !== '' && $col) {
                    $brandsContextQuery->whereHas('attributeValues', function (Builder $q) use ($attrForBrand, $val, $col) {
                        ...
                        if (is_array($val)) {
                            $c = array_filter($val, fn($v) => $v !== null && $v !== '');
                            if (!empty($c)) $q->whereIn($col, $c);
                        } else {
                            $q->where($col, $val);
                        }
                    });
    ...
    private function mapAttributeTypeToFilterInputType(string $attributeType, ?array $definedOptions = []): string
    {
        ...
        if ($hasDefinedOptions && count($definedOptions) > 0) {
            return 'checkbox';
        }
        return 'checkbox';
    }
